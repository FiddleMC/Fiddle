...
public abstract class AbstractItemMappingBuilderImpl<T, H> {
    ...
    public void registerWith(ItemMappingsComposeEventImpl event) {
        ...
        if (this.function != null) {
            event.register(this.awarenessLevels, this.getItemsToRegisterFor(), this.createFunctionStep());
            return;
        }
        if (this.to != null) {
            event.register(this.awarenessLevels, this.getItemsToRegisterFor(), new SimpleItemMappingsStep(this.getSimpleTo()));
            return;
        }
